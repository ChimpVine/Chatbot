...
from utils.homework_helper import simple_ques, image_ques, image_url_ques, mistake_finder
# For normal Chat 
...
@app.route('/find_mistake', methods=['POST'])
def find_mistake():
    user_question = request.form.get('user_question')
    if 'image' not in request.files:
        return jsonify({'error': 'No file uploaded'}), 400
    ...
    try:
        ...
        steps = mistake_finder.upload_question(base64_image,user_question)
        steps = json.dumps(steps,sort_keys=False)
        steps = json.loads(steps)
        return steps
    except Exception as e:
